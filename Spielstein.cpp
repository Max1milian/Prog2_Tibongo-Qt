...
void Spielstein::zeichne() {
  ...
  for (int i = 0; i < felder[seite][orientierung].size();
       i++) // seite bestimmt ob gespiegelt oder nicht
            //  orientierung bestimmt die rotation
  {
    ...
    // da die Konsole bei (0,0) anfängt zu zeichen, müssen wir noch ein offset
    // in form von position.getX()/.getX() draufrechnen gezeichnet wird der
    // aktuelle punkt
    Console::zeichne_punkt(
        felder[seite][orientierung].at(i).getX() + position.getX(),
        felder[seite][orientierung].at(i).getY() + position.getY(),
        CONFIGURATION::SPIELFELD_HINTERGRUND_SYMBOL);
  }
  Cursor::setze_Farbe(CURSOR_SCHWARZ);
}
void Spielstein::loesche() {
  // hier nehmen wir die gleiche Funktion wie bei zeichne(), jedoch mit der
  // abänderung, dass wir das Feld auf Schwarz setzen
  Cursor::setze_Farbe(CURSOR_SCHWARZ);
...
void Spielstein::bewegen(int richtung) {
  ...
  case CONFIGURATION::SPIELSTEIN_BEWEGEN_HOCH:
    position.setY(
        position.getY() -
        1); // der 0,0 punk ist bei der konsole oben links, (vierter Quadrant)
    break;  // deswegen müssen wir um den Spielstein nach oben zu bewegen eins
            // abziehen
...
// Inkrementiert das Attribut orientierung um 1, wobei stets Werte zwischen 0
...
void Spielstein::rotation_rechts() {
  ...
  case 3:
    orientierung = 0;
    break;
  // default lassen wird drinnen um undefined behaviour vorzubeugen falls wir
  // eine andere eingabe bekommen
  default:
    orientierung = 0;
    break;
  }
...
// Dekrementiert das Attribut orientierung um 1, wobei stets Werte zwischen 0
// und 3 angenommen werden.
...
// Inkrementiert das Attribut seite um 1, wobei stets Werte zwischen 0 und 1
// angenommen werden.
...
// Gibt true zur�ck, falls die �bergebene Position innerhalb des Spielsteines
// liegt. Ansonsten wird false zur�ckgegeben.
...
// Gibt wahr zurück, falls sich der aufrufende und der �bergebene Spielstein
// sich in mindestens einer Position �berlappen.
bool Spielstein::ueberlapp(Spielstein &sp) {
  for (int i = 0; i < felder[seite][orientierung].size(); i++) {
    for (int j = 0; j < sp.felder[seite][orientierung].size(); j++) {
      if (felder[seite][orientierung].at(i) ==
          sp.felder[seite][orientierung].at(
              j)) { //überprüft ob sich einträge im felder und spielstein vektor
                    //überlappen
        return true;
      }
    }
...
// Addiert zwei Spielsteine in ihrer aktuellen Lage und gibt die Summe als neuen
// Spielstein zur�ck.
Spielstein Spielstein::operator+(const Spielstein &rhs) const {
  Spielstein summe(
      SPIELSTEIN_FREI); // erzeugt einen neuen Spielstein auf dem stack
